...
public class TemplateException extends Exception {
    ...
    private transient boolean messageWasAlreadyPrintedForThisTrace;
    ...
    private void calculatePosition() {
        if (!positionsCalculated) {
            ...
            if (templateObject != null && templateObject.getBeginLine() > 0) {
                ...
                lineNumber = templateObject.getBeginLine();
                columnNumber = templateObject.getBeginColumn();
                endLineNumber = templateObject.getEndLine();
                endColumnNumber = templateObject.getEndColumn();
            }
    ...
    public void printStackTrace(PrintStream out) {
        _printStackTrace(out, true, true, true);
    }

    /**
     * Overrides {link Throwable#_printStackTrace(PrintWriter)} so that it will include the FTL stack trace.
     */
    @Override
     ...
     * @since 2.3.20
     */
    public void _printStackTrace(PrintWriter out, boolean heading, boolean ftlStackTrace, boolean javaStackTrace) {
        _printStackTrace(new PrintWriterStackTraceWriter(out), heading, ftlStackTrace, javaStackTrace);
    }
     ...
     * @since 2.3.20
     */
    public void _printStackTrace(PrintStream out, boolean heading, boolean ftlStackTrace, boolean javaStackTrace) {
        _printStackTrace(new PrintStreamStackTraceWriter(out), heading, ftlStackTrace, javaStackTrace);
    }

    private void _printStackTrace(StackTraceWriter out, boolean heading, boolean ftlStackTrace, boolean javaStackTrace) {
        if (heading) {
        ...
        if (javaStackTrace) {
            if (ftlStackTrace) {  // We are after an FTL stack trace
                ...
                out.println(_CoreAPI.ERROR_MESSAGE_HR);
                messageWasAlreadyPrintedForThisTrace = true;

                try {
                    out.printStandardStackTrace(this);
                } finally {
                    messageWasAlreadyPrintedForThisTrace = false;
                }
